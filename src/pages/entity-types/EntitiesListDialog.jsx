...
export const EntitiesListDialog = ({ open, onOpenChange, entityType }) => {
  ...
  const [isCreateEntityDialogOpen, setIsCreateEntityDialogOpen] =
    useState(false);
  ...
  const handleEditEntity = async (updatedEntity) => {
    try {
      ...
      setEntities((prev) =>
        prev.map((entity) =>
          entity.id === selectedEntity.id ? response.result[0] : entity
        )
      );
  ...
  const handleDeleteEntity = async () => {
    try {
      ...
      setEntities((prev) =>
        prev.filter((entity) => entity.id !== selectedEntity.id)
      );
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the entity "{selectedEntity?.label}".
